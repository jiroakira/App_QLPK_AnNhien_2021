...
import 'package:medapp/components/calendar/calendar_date_element.dart';
...
import 'package:medapp/components/utils/calendar_utils.dart';
...
class _CalendarState extends State<Calendar> with SingleTickerProviderStateMixin {
  ...
  void _initCalendar() {
    ...
    for (int i = 0;; i = i + 7) {
      ...
      if (_startOfMonth.add(Duration(days: i + 7)).month != _selectedDate.month) {
        break;
      }
    }
                  ...
                  child: _expanded
                            ...
                            children: [
                              TableRow(
                                children: _weekDayDateTimeList.map((day) {
                                  String dayInital = DateFormat('E').format(day).substring(0, 1);
                                  return Center(
                                    child: Text(
                                      '$dayInital',
                                      style: TextStyle(
                                        color: day.weekday == DateTime.now().weekday ? SchoolToolkitColors.blue : SchoolToolkitColors.lightGrey,
                                        fontWeight: FontSize.semiBold,
                                        fontSize: FontSize.fontSize20,
                                      ),
                                    ),
                                  ...
                                  children: dateList.map((date) {
                                    return Column(
                                      children: <Widget>[
                                        CalendarDateElement(
                                          date: date.day,
                                          today: date.day == DateTime.now().day && date.month == DateTime.now().month,
                                          fade: date.month != _selectedDate.month,
                                          selected: date.day == _selectedDate.day && date.month == _selectedDate.month,
                                          onTap: () {
                                            _selectDate(date);
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            color: CalendarUtils.getCalendarEventColor(
                                              date,
                      ...
                      : Table(
                          ...
                          children: [
                            TableRow(
                              children: _weekDayDateTimeList.map((day) {
                                String dayInitial = DateFormat('E').format(day).substring(0, 1);
                                return Center(
                                  child: Text(
                                    '$dayInitial',
                                    style: TextStyle(
