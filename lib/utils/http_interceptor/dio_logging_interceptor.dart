...
class DioLoggingInterceptors extends InterceptorsWrapper {
  ...
  Future onError(DioError dioError) async {
    ...
    if (oldAccessToken != null && responseCode == 401 && _sharedPreferencesManager != null) {
      ...
      RefreshTokenBody refreshTokenBody = RefreshTokenBody(refresh: refreshToken);
      // ApiAuthRepository apiAuthRepository = ApiAuthRepository();
      ApiAuthRepository apiAuthRepository = ApiAuthRepository();
      ResponseTokenBody token = await apiAuthRepository.postRefreshAuth(refreshTokenBody);
      String newAccessToken = token.access;
      String newRefreshToken = token.refresh;
